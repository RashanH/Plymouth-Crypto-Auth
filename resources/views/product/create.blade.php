    ...
    <div class="block p-6 rounded-lg shadow-lg bg-white sm:max-w-xl" style="margin: auto;">
        <form action="{{ route('products.store') }}" method="POST">
            ...
            <div class="form-group mb-6"> <input type="text"
                    ...
                    placeholder="Product name" name="name" value="{{ old('name') ?? '' }}"> </div>
            <div class="grid md:grid-cols-2 gap-4">
                <div class="form-group mb-6"> <input type="text"
                        class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                        placeholder="Latest version" name="latest_version" value="{{ old('latest_version') ?? '' }}">
                </div>
            ...
            <div class="form-group mb-6"> <input type="text"
                    class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                    id="exampleInput126" placeholder="Secret code" name="secret_code"
                    value="{{ old('secret_code') ?? '' }}"> </div>
            <div class="form-group mb-6">
                <textarea
                    class="  form-control  block  w-full  px-3  py-1.5  text-base  font-normal  text-gray-700  bg-white bg-clip-padding  border border-solid border-gray-300  rounded  transition  ease-in-out  m-0  focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                    rows="5" placeholder="Description" name="description">{{ old('description') ?? '' }}</textarea></div>
